...
# Runs in both modes, because commits made from this tree have to carry this
# setup's identity whoever owns the machine.
#
# In full mode that is the whole of ~/.config/git/config. In layer mode that
# file belongs to the base setup and is regenerated from *its* profile.env, so
# writing there would be both a violation of invariant 3 and pointless -- the
# next run of the other installer would undo it. Instead the layer writes a
# second, identity-only config and conditions it on the tree:
#
#   ~/.gitconfig             [includeIf "gitdir:<LAYER_ROOT>/"] -> the file below
#   ~/.config/git/config-<suffix>   [user] name/email
#
# ~/.gitconfig is free to use for this: full mode writes ~/.config/git/config
...
# of config.template (delta as the pager, nvim as the editor) configures tools
# the base setup may not have installed, and forcing them on inside one tree
# turns `git diff` there into "delta: command not found".
write_layer_gitconfig() {
  local included="${XDG_CONFIG_HOME}/git/config-${LAYER_SUFFIX}"

  # Invariant 5: a machine switched from layer to full must not keep an include
  # that now duplicates -- or contradicts -- the canonical config.
  if [ "${DEVSETUP_MODE}" = "full" ] || [ -z "${LAYER_ROOT:-}" ]; then
    remove_managed_block "${HOME}/.gitconfig" "GIT_IDENTITY"
    rm -f "${included}"
    return 0
  fi

  cat > "${included}" << EOF
# Generated by setup/lib/shell.sh — edit GIT_USER_* in config/profile.env.
[user]
	name = ${GIT_USER_NAME}
	email = ${GIT_USER_EMAIL}
EOF

  # The trailing slash is load-bearing: "gitdir:<dir>" matches that one
  # directory, "gitdir:<dir>/" matches everything beneath it, which is what a
  # tree of repositories needs.
  local snippet
  snippet=$(mktemp)
  cat > "${snippet}" << EOF
[includeIf "gitdir:${LAYER_ROOT}/"]
	path = ${included}
EOF
  managed_block "${HOME}/.gitconfig" "GIT_IDENTITY" "${snippet}"
  rm -f "${snippet}"
}
